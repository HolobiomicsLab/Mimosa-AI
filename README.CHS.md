...
我们宁可你在 README 里发现这些,也不愿你在生产中发现。诚实清单:

- **LLM 裁判的残余风险。**验证器的可执行声明是确定性 Python,但这些程序的*生成*以及所有软声明的裁定均来自 `judge_model`。弱模型或量化模型会产出薄弱声明。即使你在其他地方节省成本,也请为 `judge_model` 使用强模型。
- **OpenRouter 量化提供商。**部分 fp8/int4 提供商能通过基本能力检查,但会破坏转义序列(`\\n` bug) —— 生成的工作流要么崩溃要么静默地误算。如果你看到奇怪的解析错误,请绕开量化端点。参见 [Troubleshooting](./docs/reference/troubleshooting.md)。
- **作弊检测器目前已禁用。**针对生成源码的独立作弊检测通道目前为 `cheat = None`,等待重写。行为层面的反作弊压力来自 Source C 的"从磁盘重新计算"验证器、反向的"使用了 fallback"声明,以及反同义反复绊线 —— 但一个有意为之、按输出形状定制的工作流仍有可能蒙混过关。
- **行为描述符是结构性的,而非语义的。**`[n_agents, n_edges, n_branches, prompt_chars]` 概括了基因型;两个结构相同但工具路由不同的工作流会落到同一点。基于工作空间指纹或轨迹嵌入的描述符在路线图上。
- **Goal 模式仍是人写目标。**"自主"指的是工作流合成,而非问题界定。规划器分解的是*你的*目标;它不会去挑选科学问题。
- **硬性失败上限较为宽容。**`_HARD_FAIL_CAP = 0.99` 用于保持进化信号的平滑性,这意味着即便一个硬性声明被驳斥,得分仍然很高。`state_result.json` 中的 `hard_fail_capped` 标志会告诉你何时发生 —— 在报告数字前请检查它。
- **`--learn` 成本可能扩张。**最坏情况下的预算是 `max_learning_evolve_iterations × per-generation cost`。在高端模型上,若你将上限设为 35 代且始终未触发早停,请按每任务 $30–$60 进行预算。
- **Toolomics 使用了默认端口。**默认发现范围 `0.0.0.0:5000–5100` 会与常见开发服务器(Flask 等)冲突。请配置 `discovery_addresses`,或将 Toolomics 运行在不冲突的范围内。
