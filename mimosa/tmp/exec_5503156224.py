...
# Task: Find how to use “mzmind” in batch / command-line mode, then install it
# on macOS.  The workflow strictly follows the “divide-and-conquer” principle,
# uses 5 atomic agents, 3 routing functions, retry counters and two emergency
# fall-back exits.
###############################################################################

# ----- PRE-DECLARED ITEMS (already available in execution context) ----------
...
###############################################################################
# 1.  AGENT PROMPTS  (each handles ONE atomic responsibility)
###############################################################################
...
## TASK
Search the web exhaustively for information on **using "mzmind" in batch /
command-line (non-interactive) mode**.  Look for:
- Official docs / wikis
- Blog posts, user guides or forum threads
- Example batch commands or scripts

## OUTPUT FORMAT
If you find substantial, useful information:
final_answer("WEB_SEARCH_COMPLETE: [long multi-paragraph summary including \
all key findings, concrete batch examples, and full source URLs]")

If information is insufficient / unclear:
final_answer("WEB_SEARCH_FAILURE: [explain what you tried, why it failed, \
and propose alternative search ideas]")

If you hit a technical problem:
final_answer("GIVE_UP: [explain exact error]")

Only use the above three termination tags.
"""
agent_deep_web = SmolAgentFactory(prompt_deep_web, BROWSER_TOOLS)
...
## CONTEXT
The previous step’s answer (available in the conversation) contains raw search
findings about "mzmind" batch usage.

## TASK
Parse that answer and distill a **step-by-step cheat-sheet**:
1. All command-line switches / flags
2. Required input / output file formats
3. Several example batch invocations

## OUTPUT FORMAT
If extraction is successful and detailed:
final_answer("EXTRACTION_COMPLETE: [ordered list of steps, flags, examples]")

If information is missing / unclear:
final_answer("EXTRACTION_FAILURE: [detail missing pieces]")

If technical error prevents work:
final_answer("GIVE_UP: [error explanation]")
"""
agent_extractor = SmolAgentFactory(prompt_extractor, BROWSER_TOOLS)

# C) macOS Install Search -----------------------------------------------------
prompt_install_search = """
You are a web research agent.

## TASK
Find authoritative instructions for **installing "mzmind" on macOS**.
Identify:
- Package managers (Homebrew, MacPorts, dmg, etc.)
- System prerequisites / dependencies
- Exact shell commands

## OUTPUT FORMAT
If you compiled a full installation guide:
final_answer("INSTALL_SEARCH_COMPLETE: [detailed macOS installation steps \
with commands and source URLs]")

Otherwise:
final_answer("INSTALL_SEARCH_FAILURE: [what’s missing & suggested next steps]")

Technical problems:
final_answer("GIVE_UP: [error detail]")
"""
agent_install_search = SmolAgentFactory(prompt_install_search, BROWSER_TOOLS)

# D) Installer (shell executor) ----------------------------------------------
prompt_installer = """
You are an installation execution agent with shell access.

## TASK
Using the provided installation instructions, execute the commands to install
"mzmind" on macOS.  Use Homebrew or the method discovered.  Capture any output.

## COMPLETION
If installation finishes with no critical errors:
final_answer("INSTALLATION_SUCCESS: [summary of executed commands & \
important output]")

If commands fail:
final_answer("INSTALLATION_FAILURE: [error logs & suggestions]")

On technical tool issues:
final_answer("GIVE_UP: [error explanation]")
"""
agent_installer = SmolAgentFactory(prompt_installer, SHELL_TOOLS)
...
## TASK
Verify that "mzmind" is now available system-wide:
- Run `which mzmind` or equivalent
- Query version if possible

## COMPLETION
If executable is found and works:
final_answer("VALIDATION_SUCCESS: [path and version]")

If not installed / broken:
final_answer("VALIDATION_FAILURE: [observations and clues]")

Technical issues:
final_answer("GIVE_UP: [problem detail]")
"""
agent_validator = SmolAgentFactory(prompt_validator, SHELL_TOOLS)

###############################################################################
# 2.  ROUTING / ERROR-HANDLING FUNCTIONS
###############################################################################

# Helper to count how many times a node has already run
def _count_runs(state: "WorkflowState", node_name: str) -> int:
    return sum(1 for s in state.get("step_name", []) if s == node_name)

# ----- Router after DEEP WEB SEARCH -----------------------------------------
def route_after_deep_web(state: "WorkflowState") -> str:
    try:
        last_answer = state.get("answers", [])[-1] if state.get("answers") else ""
        attempts     = _count_runs(state, "deep_web_search")
        
        if "WEB_SEARCH_COMPLETE" in last_answer:
            return "usage_extractor"
        
        # Retry logic
        if attempts < 3:
            return "deep_web_search"   # retry same step
        else:
            # Too many failures – give up early
            return "end_failure"
    except Exception as e:
        print(f"Routing error after deep_web_search: {e}")
        return "end_failure"

# ----- Router after USAGE EXTRACTION ----------------------------------------
def route_after_extractor(state: "WorkflowState") -> str:
    try:
        last_answer = state.get("answers", [])[-1] if state.get("answers") else ""
        attempts     = _count_runs(state, "usage_extractor")
        
        if "EXTRACTION_COMPLETE" in last_answer:
            return "install_web_search"
        
        # Missing data?  Go back to first search as fallback
        if attempts < 2:
            return "deep_web_search"
        
        # Final fallback: stop with failure
        return "end_failure"
    except Exception as e:
        print(f"Routing error after usage_extractor: {e}")
        return "end_failure"

# ----- Router after INSTALL SEARCH / INSTALL / VALIDATE ---------------------
def route_after_install_search(state: "WorkflowState") -> str:
    try:
        last_answer = state.get("answers", [])[-1] if state.get("answers") else ""
        attempts     = _count_runs(state, "install_web_search")
        
        if "INSTALL_SEARCH_COMPLETE" in last_answer:
            return "installer"
        ...
        print(f"Routing error after install_web_search: {e}")
        return "end_failure"

def route_after_installer(state: "WorkflowState") -> str:
    try:
        last_answer = state.get("answers", [])[-1] if state.get("answers") else ""
        attempts     = _count_runs(state, "installer")
        
        if "INSTALLATION_SUCCESS" in last_answer:
            return "validator"
        
        # Retry installation once, then re-investigate instructions
        if attempts == 0:
            return "installer"                 # simple retry
        elif attempts == 1:
            return "install_web_search"        # fresh research
        else:
            return "end_failure"
    except Exception as e:
        print(f"Routing error after installer: {e}")
        return "end_failure"

def route_after_validator(state: "WorkflowState") -> str:
    try:
        last_answer = state.get("answers", [])[-1] if state.get("answers") else ""
        if "VALIDATION_SUCCESS" in last_answer:
            return END
        else:
            # Could not validate – attempt install again once
            if _count_runs(state, "installer") < 3:
                return "installer"
            return "end_failure"
    except Exception as e:
        print(f"Routing error after validator: {e}")
        return "end_failure"
...
# ---- Nodes -----------------------------------------------------------------
workflow.add_node("deep_web_search",  WorkflowNodeFactory.create_agent_node(agent_deep_web))
workflow.add_node("usage_extractor",   WorkflowNodeFactory.create_agent_node(agent_extractor))
workflow.add_node("install_web_search",WorkflowNodeFactory.create_agent_node(agent_install_search))
...
# ---- Edges -----------------------------------------------------------------
workflow.add_edge(START, "deep_web_search")

workflow.add_conditional_edges(
    "deep_web_search",
    route_after_deep_web,
    {
        "usage_extractor":    "usage_extractor",
        "deep_web_search":    "deep_web_search",  # retry path
        "end_failure":        END
    }
)
...
workflow.add_conditional_edges(
    ...
    {
        "install_web_search": "install_web_search",
        "deep_web_search":    "deep_web_search",  # fallback to first step
        "end_failure":        END
    }
)
...
workflow.add_conditional_edges(
    "install_web_search",
    route_after_install_search,
    {
        "installer":          "installer",
        "install_web_search": "install_web_search",  # retry
        "end_failure":        END
    }
)
...
workflow.add_conditional_edges(
    ...
    {
        "validator":          "validator",
        "installer":          "installer",          # simple retry
        "install_web_search": "install_web_search", # re-research
        "end_failure":        END
    }
)
...
# Ensure hard-failure node leads to END
workflow.add_node("end_failure", lambda x: x)
workflow.add_edge("end_failure", END)

# ---- Compile Workflow ------------------------------------------------------
app = workflow.compile()
###############################################################################
# The `app` object is now an executable LangGraph workflow conforming to all
# decomposition, error-handling and fallback requirements.
###############################################################################
...
initial_state = {
    "workflow_uuid": "93b8d4f76bd34d4899ad7ca6b4e06733",
    "step_name": [],
    "step_uuid": [],
    "actions": [],
    "observations": [],
...
try:
    if "workflows/93b8d4f76bd34d4899ad7ca6b4e06733":
        print("workflow run: saving workflow graph as PNG at ", "workflows/93b8d4f76bd34d4899ad7ca6b4e06733")
        try:
            png = app.get_graph().draw_mermaid_png()
            with open(os.path.join("workflows/93b8d4f76bd34d4899ad7ca6b4e06733", "workflow_93b8d4f76bd34d4899ad7ca6b4e06733.png"), "wb") as f:
                f.write(png)
        except Exception as e:
...
try:
    ...
    pass
print("workflow run: workflow execution completed for UUID:", "93b8d4f76bd34d4899ad7ca6b4e06733")
print("workflow run: result state:", result_state)

if "workflows/93b8d4f76bd34d4899ad7ca6b4e06733":
    print("workflow run: saving workflow state JSON at :", "workflows/93b8d4f76bd34d4899ad7ca6b4e06733")
    try:
        with open(os.path.join("workflows/93b8d4f76bd34d4899ad7ca6b4e06733", "state_result_93b8d4f76bd34d4899ad7ca6b4e06733.json"), "w") as f:
