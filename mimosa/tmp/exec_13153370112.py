...
# Generated workflow
# ==========================================================
#  MetaboT Installation – LangGraph · SmolAgent Workflow
# ==========================================================

# PRE-DECLARED items (already in interpreter context):
#   – WorkflowState, Action, Observation classes
#   – SHELL_TOOLS, BROWSER_TOOLS, CSV_TOOLS
#   – SmolAgentFactory
#   – WorkflowNodeFactory
#   – langgraph.graph.StateGraph, START, END
# DO **NOT** re-declare them – we simply use them.
...
# A. Web searcher – locate official sources
prompt_search = """
You are a focused web-research agent.
GOAL: Locate the official MetaboT software homepage or repository and collect:
  • Official download URL(s)
  • Latest version number
  • Any documentation pages

PROCEDURE
1. Use browser tools to query search engines and follow links.
2. Validate that the page is legitimate (check domain / repo owner).

SUCCESS OUTPUT
If you have AT LEAST one valid download link + version information:
final_answer("SEARCH_COMPLETE: <detailed summary + urls>")

FAILURE OUTPUT
If you cannot find a valid download link after multiple attempts:
final_answer("SEARCH_FAILURE: <what you tried / what failed>")

ERROR OUTPUT
For technical tool errors:
final_answer("GIVE_UP: <error details>")
"""

# B. Info extractor – pull install instructions & dependencies
prompt_extract = """
You are an information-extraction agent.
INPUT: Summary including MetaboT URLs from the previous step.
TASK
  • Open the download / documentation pages.
  • Extract clear step-by-step installation instructions.
  • List required dependencies (language runtimes, packages, lib names).

SUCCESS ⇒ final_answer("EXTRACTION_COMPLETE: <steps + dependencies>")
FAILURE ⇒ final_answer("EXTRACTION_FAILURE: <why insufficient>")
ERROR   ⇒ final_answer("GIVE_UP: <error>")
"""

# C. Dependency checker – verify system has everything
prompt_depcheck = """
You are a dependency-checking agent.
INPUT: List of required dependencies from previous step.
TOOLS: shell commands only (do NOT install yet).

For each dependency, run the appropriate shell command to confirm presence.
If ALL are present:
  final_answer("COMPATIBLE: <summary>")
If ANY are missing:
  final_answer("MISSING_DEPENDENCIES: <list missing>")
On shell errors:
  final_answer("CHECK_ERROR: <details>")
"""

# D. Dependency installer – install what’s missing
prompt_depinstaller = """
You are a dependency-installer agent.
INPUT: Missing dependencies list.

For each missing item, install using the safest package manager command
(yum, apt-get, brew, pip, etc.). Log every action.

SUCCESS ⇒ final_answer("DEPENDENCIES_INSTALLED: <what was installed>")
FAILURE ⇒ final_answer("DEPENDENCIES_INSTALL_FAILURE: <why failed>")
ERROR   ⇒ final_answer("GIVE_UP: <error>")
"""

# E. MetaboT installer
prompt_installer = """
You install MetaboT itself.
INPUTS: Official download link + install steps verified earlier.

Execute the install commands exactly, then verify executable presence.

SUCCESS ⇒ final_answer("INSTALLATION_SUCCESS: <path/version>")
FAILURE ⇒ final_answer("INSTALLATION_FAILURE: <stderr>")
ERROR   ⇒ final_answer("GIVE_UP: <error>")
"""

# F. Post-install validator
prompt_validator = """
You validate MetaboT installation.
Run 'metabot --version' or equivalent.

If command returns a version number:
  final_answer("VALIDATION_SUCCESS: <version>")
If command not found or bad output:
  final_answer("VALIDATION_FAILURE: <details>")
On shell/tool errors:
  final_answer("GIVE_UP: <error>")
"""
...
agent_depcheck    = SmolAgentFactory(prompt_depcheck,    SHELL_TOOLS)
agent_depinstall  = SmolAgentFactory(prompt_depinstaller,SHELL_TOOLS)
agent_install     = SmolAgentFactory(prompt_installer,   SHELL_TOOLS)
agent_validate    = SmolAgentFactory(prompt_validator,   SHELL_TOOLS)
...
# ---- 3.1  Add nodes -------------------------------------------------
wf.add_node("web_searcher",    WorkflowNodeFactory.create_agent_node(agent_search))
wf.add_node("info_extractor",  WorkflowNodeFactory.create_agent_node(agent_extract))
wf.add_node("dep_checker",     WorkflowNodeFactory.create_agent_node(agent_depcheck))
wf.add_node("dep_installer",   WorkflowNodeFactory.create_agent_node(agent_depinstall))
wf.add_node("metabot_installer", WorkflowNodeFactory.create_agent_node(agent_install))
wf.add_node("validator",       WorkflowNodeFactory.create_agent_node(agent_validate))

# ----------------------------------------------------------
# 4. Routing / Error-handling helpers
# ----------------------------------------------------------

MAX_RETRIES = 3    # universal retry ceiling

def _retry_count(state: WorkflowState, step: str) -> int:
    """Helper – count how many times `step` already executed."""
    return sum(1 for s in state.get("step_name", []) if s == step)

# ---- 4.1 Router after web search ------------------------------------
def route_after_search(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [])[-1] if state.get("answers") else ""
        if "SEARCH_COMPLETE" in ans:
            return "info_extractor"
        elif "SEARCH_FAILURE" in ans and _retry_count(state,"web_searcher") < MAX_RETRIES:
            return "web_searcher"        # retry
        else:
            return END                   # give up or too many retries / GIVE_UP
    except Exception as e:
        print(f"[RouterSearch] error: {e}")
        return END

# ---- 4.2 Router after extraction ------------------------------------
def route_after_extract(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [])[-1] if state.get("answers") else ""
        if "EXTRACTION_COMPLETE" in ans:
            return "dep_checker"
        elif "EXTRACTION_FAILURE" in ans and _retry_count(state,"info_extractor") < MAX_RETRIES:
            return "info_extractor"      # retry extraction
        else:
            # fallback: maybe re-search with different strategy
            rc = _retry_count(state,"web_searcher")
            return "web_searcher" if rc < MAX_RETRIES else END
    except Exception as e:
        print(f"[RouterExtract] error: {e}")
        return END

# ---- 4.3 Router after dependency check ------------------------------
def route_after_depcheck(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [])[-1] if state.get("answers") else ""
        if "COMPATIBLE" in ans:
            return "metabot_installer"
        elif "MISSING_DEPENDENCIES" in ans:
            return "dep_installer"
        else:  # CHECK_ERROR or unrecognised
            if _retry_count(state,"dep_checker") < MAX_RETRIES:
                return "dep_checker"
            else:
                return END
    except Exception as e:
        print(f"[RouterDepCheck] error: {e}")
        return END

# ---- 4.4 Router after dependency install ----------------------------
def route_after_depinstall(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [])[-1] if state.get("answers") else ""
        if "DEPENDENCIES_INSTALLED" in ans:
            return "dep_checker"   # re-check
        elif "DEPENDENCIES_INSTALL_FAILURE" in ans and _retry_count(state,"dep_installer") < MAX_RETRIES:
            return "dep_installer"
        else:
            return END
    except Exception as e:
        print(f"[RouterDepInstall] error: {e}")
        return END

# ---- 4.5 Router after MetaboT install -------------------------------
def route_after_install(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [])[-1] if state.get("answers") else ""
        if "INSTALLATION_SUCCESS" in ans:
            return "validator"
        elif "INSTALLATION_FAILURE" in ans and _retry_count(state,"metabot_installer") < MAX_RETRIES:
            return "metabot_installer"
        else:
            return END
    except Exception as e:
        print(f"[RouterInstall] error: {e}")
        return END

# ---- 4.6 Router after validation ------------------------------------
def route_after_validation(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [])[-1] if state.get("answers") else ""
        if "VALIDATION_SUCCESS" in ans:
            return END
        elif "VALIDATION_FAILURE" in ans and _retry_count(state,"metabot_installer") < MAX_RETRIES:
            # attempt reinstall then re-validate
            return "metabot_installer"
        else:
            return END
    except Exception as e:
...
# ----------------------------------------------------------
# 5. Wire edges & conditional paths
# ----------------------------------------------------------
wf.add_edge(START, "web_searcher")
...
wf.add_conditional_edges(
    "dep_checker",
    route_after_depcheck,
    {
        "metabot_installer": "metabot_installer",
...
# ----------------------------------------------------------
metabot_app = wf.compile()

# The `metabot_app` object is now a runnable LangGraph workflow
# capable of robustly searching, installing, and validating MetaboT
# with multiple retries and graceful failure termination.
...
initial_state = {
    "workflow_uuid": "3496adc3ca9543edab44241091895b2f",
    "step_name": [],
    "step_uuid": [],
    "actions": [],
    "observations": [],
...
try:
    if "workflows/3496adc3ca9543edab44241091895b2f":
        print("workflow run: saving workflow graph as PNG at ", "workflows/3496adc3ca9543edab44241091895b2f")
        try:
            png = app.get_graph().draw_mermaid_png()
            with open(os.path.join("workflows/3496adc3ca9543edab44241091895b2f", "workflow_3496adc3ca9543edab44241091895b2f.png"), "wb") as f:
                f.write(png)
        except Exception as e:
...
try:
    ...
    pass
print("workflow run: workflow execution completed for UUID:", "3496adc3ca9543edab44241091895b2f")
print("workflow run: result state:", result_state)

if "workflows/3496adc3ca9543edab44241091895b2f":
    print("workflow run: saving workflow state JSON at :", "workflows/3496adc3ca9543edab44241091895b2f")
    try:
        with open(os.path.join("workflows/3496adc3ca9543edab44241091895b2f", "state_result_3496adc3ca9543edab44241091895b2f.json"), "w") as f:
