...
# Generated workflow
# ---------------------------------------------
# LangGraph ‑ SmolAgent WORKFLOW:  “Install mzmine”
# ---------------------------------------------
#
# Assumptions
#   • SHELL_TOOLS, BROWSER_TOOLS, CSV_TOOLS are already imported tool-packages
#   • SmolAgentFactory  &  WorkflowNodeFactory  already exist in the runtime
#   • WorkflowState   &   Action/Observation schemas already exist
#
# Goal
#   Fully automate discovery, dependency checking, installation and
#   verification of the open-source LC-MS software “mzmine”.
#
# Design
#   STEP-1  web_search           → find official download URL + instructions
#   STEP-2  dependency_check     → audit local system for listed prerequisites
#   STEP-3  dependency_install   → install any missing prerequisites
#   STEP-4  installer            → download & install mzmine
#   STEP-5  verify               → launch/inspect version to confirm install
#
# Success / Failure flags (strict!)
#   SEARCH_COMPLETE, SEARCH_FAILURE
#   DEP_OK, DEP_MISSING, DEP_ERROR
#   DEPS_INSTALLED, DEPS_FAILED
#   INSTALL_COMPLETE, INSTALL_FAILURE
#   VERIFY_SUCCESS, VERIFY_FAILURE
#
# Two independent fallback mechanisms
#   • max-retry (3) per agent before hard failure
#   • dependency loop: failed install → re-run dependency loop once
#
# ---------------------------------------------
from langgraph.graph import StateGraph, START, END

# ============= 1. AGENT PROMPTS & CREATION =============
...
## TASK
Locate the latest MZmine official download page and extract:
1. Direct download URL (link to .zip/.jar or installer)
2. All explicit system requirements / dependencies (e.g., Java version)
3. Recommended installation commands or procedures
Return the data as valid JSON with keys:
{{
  "download_url": "<url>",
  "dependencies": ["dep1", "dep2", ...],
  "install_instructions": ["cmd1", "cmd2", ...],
  "verify_command": "<single shell command to validate installation>"
}}

## COMPLETION
If successful → finish with:   SEARCH_COMPLETE
If info missing/insufficient → finish with:   SEARCH_FAILURE
If technical error → finish with:  GIVE_UP
"""
agent_web = SmolAgentFactory(web_prompt, BROWSER_TOOLS)

# ---- Agent-2 : Dependency Check -------------------------------------------
dep_check_prompt = """
You are a system audit agent.

## CONTEXT
Previous agent's JSON output is provided as {{context}}.

## TASK
1. Parse the "dependencies" list.
2. For each dependency, run a shell command to test if it is installed.
3. Produce a JSON report:
{{
  "missing": ["depX", "depY"],   # empty list if none missing
  "present": ["depA", "depB"]
}}

## COMPLETION
If no dependencies missing → DEP_OK
If some missing           → DEP_MISSING
If shell/tool error       → DEP_ERROR
"""
agent_dep_check = SmolAgentFactory(dep_check_prompt, SHELL_TOOLS)

# ---- Agent-3 : Dependency Installer ---------------------------------------
dep_install_prompt = """
You are a package installer agent.

## CONTEXT
The dependency audit reported these missing packages: {{context}}

## TASK
For each missing dependency, execute appropriate shell install commands
(e.g., apt-get, brew, yum, or conda – choose automatically).

Return a JSON log:
{{
  "installed": ["pkg1","pkg2"],
  "failed": ["pkgX"]               # empty list if all succeed
}}

## COMPLETION
If every package installed → DEPS_INSTALLED
If at least one failed   → DEPS_FAILED
If shell/tool error      → GIVE_UP
"""
agent_dep_install = SmolAgentFactory(dep_install_prompt, SHELL_TOOLS)

# ---- Agent-4 : Main Installer ---------------------------------------------
install_prompt = """
You are the primary mzmine installer agent.

## CONTEXT
Download URL: {{context_download}}
Install instructions list: {{context_cmds}}

## TASK
1. Download the installer/archive to /tmp
2. Follow each install instruction.
3. Keep a detailed shell log.

## COMPLETION
On success (mzmine files placed in /opt or similar) → INSTALL_COMPLETE
If install fails                              → INSTALL_FAILURE
If shell/tool error                          → GIVE_UP
"""
...
## CONTEXT
Expected verification command: {{context}}

## TASK
Run the verification command. Capture stdout & stderr.

## COMPLETION
If command proves mzmine works (exit 0 & contains version text) → VERIFY_SUCCESS
Else                                                         → VERIFY_FAILURE
If shell/tool error                                          → GIVE_UP
"""
agent_verify = SmolAgentFactory(verify_prompt, SHELL_TOOLS)
...
# ---- Node registrations ----
workflow.add_node("web_search",          WorkflowNodeFactory.create_agent_node(agent_web))
workflow.add_node("dependency_check",    WorkflowNodeFactory.create_agent_node(agent_dep_check))
workflow.add_node("dependency_install",  WorkflowNodeFactory.create_agent_node(agent_dep_install))
workflow.add_node("installer",           WorkflowNodeFactory.create_agent_node(agent_installer))
workflow.add_node("verify",              WorkflowNodeFactory.create_agent_node(agent_verify))

# ============= 3. ROUTING FUNCTIONS ========================================

# ---------- helper ---------------------------------------------------------
def _count_occ(state: WorkflowState, step: str) -> int:
    return sum(1 for s in state.get("step_name", []) if step in s)

# ---------- Router-A : after web_search ------------------------------------
def router_web(state: WorkflowState) -> str:
    try:
        ...
        if "GIVE_UP" in answer:
            return END
        # retry logic
        if _count_occ(state, "web_search") < 3:
            return "web_search"
        # exceeded retries
        return END
    except Exception as e:
        print(f"[router_web] error {e}")
        return END

# ---------- Router-B : after dependency_check ------------------------------
def router_dep_check(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [""])[-1]
        if "DEP_OK" in ans:
            return "installer"
        if "DEP_MISSING" in ans:
            return "dependency_install"
        if "DEP_ERROR" in ans:
            # retry once, then abort
            if _count_occ(state, "dependency_check") < 3:
                return "dependency_check"
            return END
        # fallback safety
        return END
    except Exception as e:
        print(f"[router_dep_check] error {e}")
        return END

# ---------- Router-C : after dependency_install ----------------------------
def router_dep_install(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [""])[-1]
        if "DEPS_INSTALLED" in ans:
            return "installer"
        if "DEPS_FAILED" in ans:
            # one more install attempt allowed
            if _count_occ(state, "dependency_install") < 2:
                return "dependency_install"
            return END
        if "GIVE_UP" in ans:
            return END
        ...
        print(f"[router_dep_install] error {e}")
        return END

# ---------- Router-D : after installer -------------------------------------
def router_installer(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [""])[-1]
        if "INSTALL_COMPLETE" in ans:
            return "verify"
        if "INSTALL_FAILURE" in ans:
            # maybe dependencies were missed → run dep loop once more then retry
            if _count_occ(state, "installer") < 3:
                return "dependency_check"
            return END
        if "GIVE_UP" in ans:
            return END
        ...
        print(f"[router_installer] error {e}")
        return END

# ---------- Router-E : after verify ----------------------------------------
def router_verify(state: WorkflowState) -> str:
    try:
        ans = state.get("answers", [""])[-1]
        if "VERIFY_SUCCESS" in ans:
            return END
        if "VERIFY_FAILURE" in ans:
            # reinstall attempt (once)
            if _count_occ(state, "installer") < 3:
                return "installer"
            return END
        if "GIVE_UP" in ans:
...
workflow.add_conditional_edges("web_search",
                               router_web,
                               {
...
workflow.add_conditional_edges("dependency_check",
                               router_dep_check,
                               {
                                   "installer": "installer",
...
workflow.add_conditional_edges("dependency_install",
                               router_dep_install,
                               {
                                   "installer": "installer",
                                   "dependency_install": "dependency_install",
...
workflow.add_conditional_edges("installer",
                               router_installer,
                               {
                                   "verify": "verify",
                                   "dependency_check": "dependency_check",
...
initial_state = {
    "workflow_uuid": "81086c60485c46e49a20589f5761dd52",
    "step_name": [],
    "step_uuid": [],
    "actions": [],
    "observations": [],
...
try:
    if "workflows/81086c60485c46e49a20589f5761dd52":
        print("workflow run: saving workflow graph as PNG at ", "workflows/81086c60485c46e49a20589f5761dd52")
        try:
            png = app.get_graph().draw_mermaid_png()
            with open(os.path.join("workflows/81086c60485c46e49a20589f5761dd52", "workflow_81086c60485c46e49a20589f5761dd52.png"), "wb") as f:
                f.write(png)
        except Exception as e:
...
try:
    ...
    pass
print("workflow run: workflow execution completed for UUID:", "81086c60485c46e49a20589f5761dd52")
print("workflow run: result state:", result_state)

if "workflows/81086c60485c46e49a20589f5761dd52":
    print("workflow run: saving workflow state JSON at :", "workflows/81086c60485c46e49a20589f5761dd52")
    try:
        with open(os.path.join("workflows/81086c60485c46e49a20589f5761dd52", "state_result_81086c60485c46e49a20589f5761dd52.json"), "w") as f:
