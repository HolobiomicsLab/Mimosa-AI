...
これらを本番環境ではなく README で見つけていただきたいと考えています。正直なリスト:

- **LLM-judge による残存リスク.** verifier の実行可能クレームは決定論的 Python ですが、それらのプログラムの *生成* とすべてのソフトクレーム判定は `judge_model` から来ます。弱いまたは量子化された judge モデルは弱いクレームを生み出します。他をケチっても `judge_model` には強力なモデルを使ってください。
- **OpenRouter の量子化プロバイダ.** 一部の fp8/int4 プロバイダは基本的な能力チェックを通過しますが、エスケープシーケンスを破損させます（`\\n` バグ）。生成されたワークフローがクラッシュするか、静かに誤計算します。奇妙なパースエラーが見られる場合は、量子化エンドポイントを迂回してください。[Troubleshooting](./docs/reference/troubleshooting.md) を参照してください。
- **チート検出器は無効化されています.** 生成されたソースに対するスタンドアロンのチート検出パスは、書き直しを待って現在 `cheat = None` です。挙動的なアンチチート圧力は、ソース C のディスクから再計算する verifier、反転された "Used fallback" クレーム、反トートロジートリップワイヤから来ますが、決意の固い出力形状のワークフローはまだすり抜ける可能性があります。
- **行動記述子は構造的であり、意味的ではありません.** `[n_agents, n_edges, n_branches, prompt_chars]` は遺伝子型を要約します。構造が同一でツールルーティングが異なる 2 つのワークフローは同じ点に着地します。ワークスペースフィンガープリントやトレース埋め込み記述子はロードマップ上にあります。
- **goal モードは人間が記述する目標です.** 「自律的」はワークフロー合成に適用され、問題設定には適用されません。planner は *あなたの* 目標を分解しますが、科学的な問いを選定するわけではありません。
- **ハードフェイル上限は寛容です.** `_HARD_FAIL_CAP = 0.99` は進化シグナルをなめらかに保ちますが、これは反証されたハードクレームでも高スコアになることを意味します。`state_result.json` の `hard_fail_capped` フラグがこれが発生したことを示します。数値を報告する前に確認してください。
- **`--learn` のコストはスケールしうる.** 最悪ケースの予算は `max_learning_evolve_iterations × 世代あたりコスト` です。プレミアムモデルで 35 世代まで上限を設定し早期停止しきい値に到達しない場合、タスクあたり $30 〜 $60 を見込んでください。
- **デフォルトポートの Toolomics.** 検出のデフォルト `0.0.0.0:5000–5100` は一般的な開発サーバ（Flask など）と衝突します。`discovery_addresses` を設定するか、Toolomics を競合しない範囲で起動してください。
