...
class PlannerAgent(Agent):
    ...
    async def make_plan(self, prompt: str) -> Tuple[str, str]:
        """
        ...
        return self.parse_agent_tasks(answer), answer
    ...
    async def update_plan(self, goal: str, agents_tasks: List[dict], agents_work_result: dict, id: str, success: bool) -> dict:
        ...
        plan, answer = await self.make_plan(update_prompt)
        if plan == []:
    ...
    async def process(self, goal: str) -> Tuple[str, str]:
        ...
        agents_tasks, answer = await self.make_plan(goal)
