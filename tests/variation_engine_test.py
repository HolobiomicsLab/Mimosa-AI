...
def test_iters_since_improvement_empty_history_is_zero():
    ve = VariationEngine(config=Config())
...
def test_mutation_prompt_does_not_touch_gradient_history():
    ve = VariationEngine(config=Config())
    # This test verifies gradient-history invariance, not live LLM behaviour;
    # stub the directive call so it never hits the network.
    ve.llm_think_mutation_directive = lambda *a, **k: "stub directive"
