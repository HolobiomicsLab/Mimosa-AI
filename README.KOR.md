...
<h1 align="center">Mimosa-AI — 자율 과학 연구를 위한 진화형 멀티에이전트 프레임워크</h1>
...
Mimosa-AI는 **자율 과학 연구를 위한 오픈소스 Python 프레임워크**입니다. **태스크마다 맞춤형 멀티에이전트 워크플로우를 작성**하고 이를 샌드박스에서 실행한 뒤, 에이전트가 실제로 수행한 작업을 독립적인 관점들에 비추어 검증하고, **Quality-Diversity** 기반 탐색을 통해 세대를 거쳐 워크플로우를 진화시켜 해당 태스크에 최적인 워크플로우를 찾아냅니다.

워크플로우는 순수 Python으로 출력됩니다 — DSL 없이, YAML 없이 — 그래서 어떤 세대든 검사하거나 비교하거나 단독으로 다시 실행할 수 있습니다. 검증기는 에이전트가 생성한 산출물에 대해 문헌 그라운딩, 비자명성, 품질 메트릭을 확인하는 결정론적 Python 검사를 실행하여 워크플로우를 평가합니다. 모든 세대는 그 계보와 해당 코드를 생성한 정확한 LLM 프롬프트와 함께 디스크에 저장됩니다.
...
참고: 이 명령은 동반 프로젝트인 `Toolomics`와 `Perspicacité`를 자동으로 설치하고 구동합니다.

수동 설치는 [수동 설치](##수동-설치)를 참조하세요.

---

## 웹 인터페이스

브라우저에서 `http://localhost:5173/`을 열어 웹 인터페이스에 접속하세요.
...
이 데모는 V1으로 수행되었으며 곧 업데이트될 예정입니다.

<p align="center">
    <em>Mimosa-AI는 <a href="https://www.researchgate.net/publication/323525305_Bioactivity-Based_Molecular_Networking_for_the_Discovery_of_Drug_Leads_in_Natural_Product_Bioassay-Guided_Fractionation">Nothias et al. (2018)</a>의 LC-MS/MS 분자 네트워킹 파이프라인 — <code>.mzML</code> 파일에 대한 피처 검출(MZmine / OpenMS / matchms 계열 도구 스택 — 에이전트가 직접 선택), 정렬, 그리고 고전적 분자 네트워킹(GNPS 스타일 코사인 클러스터링) — 을 자율적으로 재생성했습니다.</em>
</p>
...
- **선택: Quality-Diversity 아카이브** — **비구조 아카이브**(이산 그리드가 아니라 단일 목록), 인구 상한 20, 단일 스칼라화 점수 `qd_score = (1−w)·quality + w·novelty` (`w = novelty_weight = 0.25`)로 평가하며, 가득 차면 가장 낮은 `qd_score` 멤버를 퇴출합니다. **참신성 탐색 (novelty search)** 은 **유전자형 임베딩 (genotype-embedding)** 행동 기술자 — 워크플로우의 생성 소스 코드를 L2 정규화한 임베딩(기본값은 로컬 `all-MiniLM-L6-v2`, 선택적으로 OpenAI `text-embedding-3-small`) — 상의 코사인 거리 k-NN(`k=15`)을 사용합니다. 부모는 자식 수의 역수에 비례하는 룰렛으로 선택되어 아카이브가 고르게 퍼지도록 합니다 (`MAX_CHILDREN_PER_PARENT = 8`).
- **변이: Rechenberg-1/5 + 정체 기반 범위 조절** — 변이 대담함은 최근 5개 평가된 자식의 성공률(Rechenberg 1/5 규칙, 임계값 `0.20`)과 `iters_since_improvement` 정체 카운터(허용치 `6`)를 혼합합니다. 거의 승리한 개체(부모 점수 > 0.95)는 감쇠를 받습니다. 범위 대역은 `EXPLOITATION`(점 변이)부터 `RE-SPECIATION`(완전 재설계)까지이며, 유효 대담함 임계값(`< 0.35 / 0.50 / 0.65 / 0.90 / 1.01`)에 따라 결정됩니다.
- **교차** — 기본적으로 약 40%의 세대에서 두 부모를 결합하며, 강한 쪽이 먼저 적용되고, 자식은 가장 높은 부모의 에이전트 수로 상한이 정해집니다.
- **콜드 스타트** — 아카이브가 비어 있으면, 디스크의 과거 실행을 유사도 필터링(MiniLM 코사인 ≥ 0.8)으로 스캔하여 탐색을 시드합니다. 유용한 워크플로우는 태스크 간에 전이됩니다.
...
각 클레임은 심판(judge)이 워크스페이스에 대해 작성하는 **Python 프로그램**으로 검증됩니다 — 에이전트를 믿느냐고 LLM에 다시 묻는 방식이 아닙니다.
...
- **가장 쉬운 경로:** 동반 플랫폼 **[Toolomics](https://github.com/HolobiomicsLab/toolomics)** 설치 — 워크스페이스별 MCP 셸 샌드박스로 에이전트가 필요할 때 과학 패키지를 설치할 수 있고, 일반적인 과학 도구 스택을 노출하는 사전 빌드된 MCP 서버, 공유 워크스페이스 관리, 그리고 새로운 MCP 등록을 위한 쉬운 흐름을 함께 제공합니다.
...
**또는 독립형 `mimosa` 명령으로 설치**하여 어떤 디렉토리에서든 사용할 수 있습니다:

```bash
uv tool install git+https://github.com/HolobiomicsLab/Mimosa-AI.git   # 또는: uv tool install /path/to/Mimosa-AI
mimosa
```

이 방식으로 설치하면 설정은 `~/.config/mimosa/config.json`에 저장되고(온보딩 마법사가 작성, 매 실행 시 자동 로드), API 키는 `~/.config/mimosa/.env`에, 런타임 상태(메모리, 워크플로우, 실행 캡슐)는 `~/.local/share/mimosa/`에 저장됩니다. 저장소 체크아웃은 기존 레이아웃을 유지합니다: `config_default.json`과 상태 디렉토리가 체크아웃 내에 위치합니다.

전체 빠른 시작: [`docs/getting-started/quickstart.md`](./docs/getting-started/quickstart.md).

### 3. (선택) Perspicacité를 통한 과학적 그라운딩
...
export DEEPSEEK_API_KEY="xxxxx" # API 키를 내보내세요; anthropic 및 openrouter도 지원됩니다
...
```bash
cd webui && ./deploy.sh    # 의존성 설치, 백엔드 + 프론트엔드 실행; http://localhost:5173 열기
```

자세한 내용, 환경 변수, 전체 API 목록은 다음을 참고하십시오:
...
참조: [`docs/usage/transparency.md`](./docs/usage/transparency.md), [`docs/usage/workspace.md`](./docs/usage/workspace.md).

---

## 설정

참조: [`docs/reference/configuration.md`](./docs/reference/configuration.md).
...
| [ELM](https://github.com/CarperAI/OpenELM) (Lehman et al. 2022) | LLM 매개 코드에 대한 quality-diversity | 가장 가까운 QD 조상. Mimosa의 행동 기술자는 도메인 특화 핸드디자인 대신 워크플로우 코드 자체의 도메인 독립적 임베딩입니다 |
...
```bash
uvx --with mkdocs-material mkdocs serve   # http://localhost:8000에서 실시간 미리보기
uvx --with mkdocs-material mkdocs build   # ./site에 정적 HTML 생성
```
...
Apache 2.0. [`NOTICE`](./NOTICE), [`docs/licensing-notes.md`](./docs/licensing-notes.md), 그리고 기여 조항에 대해서는 [`CLA/`](./CLA/) 폴더를 참조하세요.
