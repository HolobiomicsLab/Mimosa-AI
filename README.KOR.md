...
이런 항목은 프로덕션이 아니라 README에서 발견하시길 바랍니다. 솔직한 목록:

- **LLM 심판 잔여 위험.** 검증기의 실행 가능한 클레임은 결정론적 Python이지만, 그 프로그램의 *생성*과 모든 소프트 클레임 판정은 `judge_model`에서 나옵니다. 약하거나 양자화된 심판 모델은 약한 클레임을 생성할 수 있습니다. 다른 곳에서 비용을 아끼더라도 `judge_model`에는 강한 모델을 사용하세요.
- **OpenRouter 양자화 제공자.** 일부 fp8/int4 제공자는 기본 능력 검사는 통과하지만 이스케이프 시퀀스를 손상시킵니다(`\\n` 버그) — 생성된 워크플로우가 크래시되거나 조용히 잘못 계산합니다. 이상한 파싱 오류가 보이면, 양자화된 엔드포인트를 우회하세요. [Troubleshooting](./docs/reference/troubleshooting.md)을 참조하세요.
- **부정 탐지기는 비활성화되어 있습니다.** 생성된 소스에 대한 독립적인 부정 탐지 패스는 현재 재작성을 대기하며 `cheat = None`입니다. 행동 기반 반(反)부정 압력은 Source C의 디스크 재계산 검증기, 반전된 "Used fallback" 클레임, 반(反)동어반복 트립와이어에서 나오지만 — 작정한 출력 형태의 워크플로우는 여전히 빠져나갈 수 있습니다.
- **행동 기술자는 의미가 아니라 구조 기반입니다.** `[n_agents, n_edges, n_branches, prompt_chars]`는 유전자형을 요약합니다. 구조는 동일하지만 도구 라우팅이 다른 두 워크플로우는 같은 지점에 위치하게 됩니다. 워크스페이스 핑거프린트나 추적 임베딩 기반 기술자는 로드맵에 있습니다.
- **Goal 모드는 사람이 작성한 목표를 사용합니다.** "자율"은 워크플로우 합성에 적용되며 문제 정의에는 적용되지 않습니다. 플래너는 *사용자의* 목표를 분해할 뿐, 과학적 질문을 선택하지는 않습니다.
- **하드 페일 상한은 관대합니다.** `_HARD_FAIL_CAP = 0.99`는 진화 신호를 매끄럽게 유지하는데, 이는 반박된 하드 클레임도 여전히 높은 점수를 받는다는 의미입니다. `state_result.json`의 `hard_fail_capped` 플래그는 이런 일이 발생했음을 알려줍니다 — 수치를 보고하기 전에 확인하세요.
- **`--learn` 비용은 확장될 수 있습니다.** 최악의 경우 예산은 `max_learning_evolve_iterations × per-generation cost`입니다. 35세대까지 상한을 두고 조기 종료 임계값에 도달하지 못할 경우, 프리미엄 모델에서는 태스크당 $30–$60를 예상하세요.
- **기본 포트의 Toolomics.** 기본 탐색 범위 `0.0.0.0:5000–5100`은 일반적인 개발 서버(Flask 등)와 충돌합니다. `discovery_addresses`를 설정하거나 Toolomics를 충돌하지 않는 범위에서 실행하세요.
