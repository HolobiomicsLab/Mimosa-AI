...
- **Python Execution by Default**: Agents possess a built-in Python execution capability. If an agent is defined without a specific tool package, it will default to generating and executing Python code to achieve its objective.
- **Prioritize Python for General Tasks**: For most sub-tasks that require coding, utilize a tool-less agent that leverages its default Python capability. Reserve agents with specialized tools (e.g., R or C execution) for domain-specific applications like scientific computing or low-level system manipulation.
