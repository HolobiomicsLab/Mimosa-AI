...
Start from the task, not from a menu of shapes. There are no fixed patterns:
design the smallest system the task's own requirements justify.

- Begin with a single agent. Add a role only when the mutation directive or a
  diagnosed failure mode names the specific gap that role fills - never as
  boilerplate.
- Topology is a variable, not a tradition. If previous attempts converged on
  the same structure and still failed, the next design must change roles,
  routing, or stages - not merely reword the same agents.
- Checks stay adversarial, not redundant: a checker NEVER recomputes the
  solver's analysis. It interrogates the result and logs against the task's
  stated success conditions - is the threshold reached, does the value hold,
  is the deliverable present and non-degenerate - and raises a falsifying
  question a self-report would miss.
- Avoid shared blind spots: give validator, critic, and solver roles different
  models where possible, and only deliberate when the critics are grounded in
  different evidence or run on different models.
...
- Verification must be adversarial, not deliberative and not recomputing. A checker interrogates the result and logs against the task's stated success conditions - threshold reached, value held, deliverable present and non-degenerate - and raises falsifying questions a self-report would miss. It never re-runs the solver's analysis and never reads source code.
...
- Add an agent only when it forces a commitment the single pass skips or routes around a real bottleneck.
