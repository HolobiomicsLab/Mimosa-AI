...
def master_router(state: WorkflowState) -> str:
    ...
    if "SUCCESS" in last_answer.status or "SUCCESS" in last_answer.answer:
        print(f"✅ Success from '{current_agent}'. Proceeding.")
        return "next_node"
    elif "FALLBACK" in last_answer.status or "FALLBACK" in last_answer.answer:
    ...
    elif "RETRY" in last_answer.status or "RETRY" in last_answer.answer:
        retry_count = state["step_name"][-5:].count(current_agent)
